...
class OutlierImputer(BaseEstimator, TransformerMixin):
    def __init__(self, feat_with_outliers=['Income (INR)', 'Loan Amount Request', 'Current Loan Expenses', 'Property Price']):
...
class MissingValueImputer(BaseEstimator, TransformerMixin):
    def __init__(self, mode_imputed_ft=['Gender', 'Income Stability', 'Dependents', 'Has Active Credit Card', 'Property Location'], median_imputed_ft=['Income (INR)', 'Current Loan Expenses (INR)', 'Credit Score']):
...
class DropFeatures(BaseEstimator, TransformerMixin):
    def __init__(self, feature_to_drop=['Customer ID', 'Name', 'Type of Employment']):
...
class ValueImputer(BaseEstimator, TransformerMixin):
    def __init__(self, feat_with_999_val=['Co-Applicant', 'Loan Sanction Amount', 'Property Price']):
...
class MinMaxWithFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, min_max_scaler_ft=['Age', 'Income (INR))', 'Loan Amount Request', 'Credit Score', 'Property Price']):
...
class OneHotWithFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, one_hot_enc_ft=['Gender', 'Profession', 'Location', 'Has Active Credit Card', 'Property Location', 'Income Stability','Co-Applicant']):
...
class SkewnessHandler(BaseEstimator, TransformerMixin):
    def __init__(self, col_with_skewness=['Income (INR)', 'Loan Amount Request']):
...
## Income
""")
input_income = st.slider('Select your income ', value=2500000,
                         min_value=0, max_value=50000000, step=10)
...
## Profession
""")
professions = ['Student', 'Self-Employed', 'Pensioner', 'Not working','Governement employee','Salaried official']
...
prop_price = st.slider('Select your property price', value=0,
                       min_value=0, max_value=30000000, step=100)
...
loan_amount_req = st.slider('Select your current loan expenses', value=0,
                            min_value=0, max_value=500000, step=100)
...
#profile_to_pred_prep = full_pipeline(
 #   train_copy_with_profile_to_pred).tail(1).drop(columns=['Loan Sanction Amount (INR)'])
...
if predict_bt:
    ...
    st.info(
        '## You have been approved for a loan amount of {0:.2f} INR'.format(final_pred[0]))
    st.balloons() 
