...
full_data = pd.read_csv(
    "https://github.com/moulya16/RiskAnalysis/blob/main/train(1).csv"
 )
...
class OutlierImputer(BaseEstimator, TransformerMixin):
    def __init__(self, feat_with_outliers=['Income (INR)', 'Loan Amount Request (INR)', 'Current Loan Expenses (INR)', 'Property Price']):
...
class ValueImputer(BaseEstimator, TransformerMixin):
    def __init__(self, feat_with_999_val=['Co-Applicant', 'Loan Sanction Amount (INR)', 'Property Price']):
...
class MinMaxWithFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, min_max_scaler_ft=['Age', 'Income (INR))', 'Loan Amount Request (INR)', 'Credit Score', 'Property Price']):
...
class OneHotWithFeatNames(BaseEstimator, TransformerMixin):
    def __init__(self, one_hot_enc_ft=['Gender', 'Profession', 'Location', 'Has Active Credit Card', 'Property Location', 'Income Stability']):
...
class SkewnessHandler(BaseEstimator, TransformerMixin):
    def __init__(self, col_with_skewness=['Income (INR)', 'Loan Amount Request (INR)']):
...
## Income
""")
input_income = st.slider('Select your income ', value=2500,
...
## Profession
""")
professions = ['Student', 'Businessman', 'Pensioner', 'Not working']
